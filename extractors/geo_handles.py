...
def kb_run_geofunction(doc_id: str, function_name: str, args_json: str = "{}") -> str:
    ...
    Loads the block's code (get_kb_block), defines `function_name`, and calls it with
    `args_json` (a JSON object). (Geo)DataFrame parameters must be passed as file_ids
    (from a prior step); the result is written to a new file_id (or a PNG file_id for a
    plot). Returns JSON with the produced file_id."""
    import json
    ...
    ns: dict = {}
    try:
        exec(_GEO_PRELUDE + _strip_tool_decorators(code), ns)
...
__all__ = ["write_geodata", "read_geodata", "capture_current_fig", "make_file_handle_tool",
           "kb_run_geofunction", "kb_select_rows", "kb_point_heatmap", "make_geo_analysis_tools"]
